  ...
  ({
    ...
    if (editing) {
      return (
        <>
          {ReactDOM.createPortal(
            <form id={formId} onSubmit={onSubmitEdit} />,
            document.body
          )}
      ...
      return (
        <span className="flex">
          <p>{value}</p>
          {warnValidation(value) && (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
              fill="currentColor"
              className="bi bi-exclamation-triangle ml-0.5 mt-0.5 text-yellow-500"
              viewBox="0 0 16 16"
            >
              <title>{warnValidation(value)}</title>
