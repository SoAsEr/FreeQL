...
import useWindowSize from "../../utils/useMediaQuery.js";
import { getEquilibria } from "./equilibriaSlice.js";
import { pending } from "../loading/loadingSlice.js";
import SpinnerComponentRow from "../../reusable_components/Spinner/Animated.js";
...
const ConcentrationTable = React.memo(
  ({ context, equilibrium, style, className }) => {
      ...
      () => [
        ...
        },
        {
          ...
          accessor: ({ concentration }) =>
            numberToExpWithTrailing(concentration, 4),
        },
        {
          Header: () => <>&minus;log(Conc)</>,
          id: "negLogConc",
          accessor: ({ concentration }) =>
            numberToExpWithTrailing(-Math.log10(concentration), 4),
        },
      ],
      []
    );
    ...
    const data = useMemo(
      () =>
        Immutable.List([
          Immutable.List(
            equilibrium.species.component.map(
              ({ componentId, concentration }) => ({
                id: componentId,
                concentration,
                db: context.componentDB.components,
              })
            )
          ),
          Immutable.List(
            equilibrium.species.aqueous.map(({ id, concentration }) => ({
              id,
              concentration,
              db: context.speciesDB.aqs,
            }))
          ).sortBy(({ id }) => context.speciesDB.aqs.get(id).index),
          Immutable.List(
            equilibrium.species?.solid?.present?.map(
              ({ id, concentration }) => ({
                id,
                concentration,
                db: context.speciesDB.solids,
              })
            )
          ).sortBy(({ id }) => context.speciesDB.solids.get(id).index),
        ]).flatten(true),
      [equilibrium, context]
    );
    const borderLengths = useMemo(
      () =>
        Immutable.Set(
          cumulativeSum([
            equilibrium.species.component.length,
            equilibrium.species.aqueous.length,
            equilibrium.solid?.present?.length ?? 0,
          ])
        ).filter((item) => item !== data.size && item !== 0),
      [equilibrium, data.size]
    );
    ...
    return (
      <ReactTable
        ...
        data={data}
        headerColumn={"name"}
        getCellProps={useCallback((cell) => {
          return borderLengths.has(cell.row.index + 1)
...
const TotalConcentrationTable = React.memo(
  ({ context, equilibrium, style, className }) => {
    const columns = useMemo(
      () => [
        {
    ...
    const data = useMemo(
      () =>
        Immutable.List(
          equilibrium.totalConcentrations.map(({ componentId, total }) => ({
            id: componentId,
            total,
            db: context.componentDB.components,
          }))
        ).sortBy(({ id }) => context.componentDB.components),
      [context, equilibrium]
    );
...
const SolublilityProductTable = React.memo(
  ({ context, equilibrium, style, className }) => {
    const columns = useMemo(
      () => [
        {
        ...
        },
        {
          ...
          accessor: ({ solubilityProduct }) =>
            numberToExpWithTrailing(solubilityProduct, 4),
        },
      ],
      []
    );
    ...
    const data = useMemo(
      () =>
        Immutable.List(
          equilibrium.species.solid.notPresent.map(
            ({ id, solubilityProduct }) => ({
              id,
              solubilityProduct,
              db: context.speciesDB.solids,
...
const ExtraSolublilityProductTable = React.memo(
  ({ context, equilibrium, style, className }) => {
    const columns = useMemo(
      () => [
        {
    ...
    const data = useMemo(
      () =>
        Immutable.List(
          equilibrium.extraSolubilityProducts.map(
            ({ id, solubilityProduct }) => ({
              id,
              solubilityProduct,
              db: context.speciesDB.solids,
          ...
          .sortBy(({ solubilityProduct }) => solubilityProduct)
          .reverse(),
      [equilibrium, context]
    );
    ...
    return (
      <ReactTable
        ...
        getRowProps={useCallback((row) => {
          return Number(row.values.solubilityProduct) > 1
      ...
      <>
        {equilibrium.result.equilibrium && (
          <ResultTables
            ...
            equilibrium={equilibrium.result.equilibrium}
          />
        )}
        {equilibrium.result.errors && (
          <ResultErrors
            context={equilibrium.context}
            errors={equilibrium.result.errors}
          />
        )}
      </>
    );
      ...
      <Footer>
        <nav className="w-full">
          <ReactPaginate
            ...
            pageClassName={
              "px-1 h-36 min-w-36 rounded-md color-gray-400 hover:bg-gray-200 text-center select-none"
            }
            previousClassName={
              "px-1 h-36 min-w-36 rounded-md color-gray-400 text-center select-none"
            }
            nextClassName={
              "px-1 h-36 min-w-36 rounded-md color-gray-400 text-center select-none"
            }
            ...
            activeClassName={
              "!bg-lightBlue-500 !color-white !hover:bg-lightBlue-500"
            }
